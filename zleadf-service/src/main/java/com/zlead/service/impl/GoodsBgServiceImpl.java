    ...
    public List<Map<String, Object>> newShowGoodsByCat(Long factoryId, Long agentId) {
        ...
        if (CollectionUtils.isNotEmpty(crmPrdCats)) {
            for (CrmPrdCat crmPrdCat : crmPrdCats) {
                Map<String, Object> map = new HashMap<>();
                catIds.add(crmPrdCat.getCatId().longValue());
                List<CrmPrdCat> childCats = null;
                if (CollectionUtils.isNotEmpty(catIds)){
                    //查找6个代理商关联的二级分类
                    childCats = crmPrdCatService.upCatIdQueryConTent(agentId, catIds, factoryId);
                }else{
                    childCats = new ArrayList<CrmPrdCat>();
                }


                // List<Map<String, Object>> childCats = oaFactoryInfoService.findAllChildCatsByPactId(factoryId, crmPrdCat.getCatId(), lMap.keySet());
                if (childCats.size() > 6) {
                    map.put("childCats", childCats.subList(0, 6));
                } else {
                    map.put("childCats", childCats);
                ...
                // List<Map<String, Object>> goods = goodsDao.findList(agentId, bMap.keySet(), lMap.keySet(), mMap.keySet(), ids, null, false,0, 8);
                PageBounds rowBounds = new PageBounds(0, 8);
                List<GoodsEntity> goodsEntities = goodsDao.newFacGetGoodsByCatIdWithAgent(catIds, agentId.intValue(), factoryId,rowBounds);
                ...
                if (crmPrdCat.getCatId().longValue() == crmPrdCat.getCatId().longValue()) {
                    catIds.remove(crmPrdCat.getCatId().longValue());
                }
