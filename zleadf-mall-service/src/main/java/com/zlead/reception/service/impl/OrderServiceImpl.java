...
//                }
                ...
                if (goodsPrice.compareTo(BigDecimal.ZERO) == -1 || goodsPrice.compareTo(BigDecimal.ZERO) == 0) {
                    ...
                    orderGoods.setOrigin_price(goods.getSupplyPrice());
                  //如果是成本价，则设置折扣为1
                    agentDiscount=BigDecimal.valueOf(1);
                ...
                if (orderEntity.getBuyType() == Cnst.OrderCnst.BUY_TYPE_1) { // 线上购买商品
                    ...
                    order = addOrderEntity(orderEntity, goods.getShopId(), shopName, memberEntity, memberAddressEntity, goodsAmount, payPoint, agentFac);
                ...
                for (int i = 0; i < shopIds.size(); i++) {
                    ...
                    if (tempOrderEntity.getBuyType() == Cnst.OrderCnst.BUY_TYPE_1) { // 线上购买商品
                        ...
                        order =  addOrderEntity(tempOrderEntity, shopIds.get(i), shopName,
                                memberEntity, memberAddressEntity, goodsAmount, payPoint, agentFac);
                    ...
                    if (order != null) {
                        ...
                        int count =0;
                        String oriPriceType = tempOrderEntity.getPriceType();
                        for (ShoppingCartEntity cartEntity : shoppingCartEntities) {
                            GoodsEntity goods = goodsDao.findById(cartEntity.getGoodsId());
                            if (goods != null) {
                                if (shopIds.get(i).equals(goods.getShopId())) {
                                    ...
                                    BigDecimal goodsPrice = new BigDecimal(0);
                                    if (goods.getProdType() == 0) {
                                        if (agentDiscountType.equals("1") && goods.getAgentPrice() != null) {
                                            goodsPrice = goods.getAgentPrice().multiply(agentDiscount);//经销价
                                            originPrice = goods.getAgentPrice();
                                            orderEntity.setPriceType("1");
                                        } else if (agentDiscountType.equals("2") && goods.getPrice() != null) {
                                            goodsPrice = goods.getPrice().multiply(agentDiscount);//批发价
                                            originPrice = goods.getPrice();
                                            orderEntity.setPriceType("2");
                                        } else if (agentDiscountType.equals("3") && goods.getMarketPrice() != null) {
                                            goodsPrice = goods.getMarketPrice().multiply(agentDiscount);//零售价
                                            originPrice = goods.getMarketPrice();
                                            orderEntity.setPriceType("3");
                                        }
                                    } else if (goods.getProdType() == 2) {
                                        goodsPrice = goods.getShowPrice();
                                        originPrice = goods.getShowPrice();
                                    	//活动价格的折扣为1
                                        agentDiscount=BigDecimal.valueOf(1);
                                    ...
                                    if (goodsPrice.compareTo(BigDecimal.ZERO) == -1 || goodsPrice.compareTo(BigDecimal.ZERO) == 0) {
                                        goodsPrice = goods.getSupplyPrice();
                                        originPrice = goods.getSupplyPrice();
                                    ...
                                    //对商品单价进行四舍五入
                                    goodsPrice = goodsPrice.setScale(2,BigDecimal.ROUND_HALF_UP);
                                    originPrice = originPrice.setScale(2, BigDecimal.ROUND_HALF_UP);
                                    ...
                                    //订单商品的信息
                                    addOrderGoods(goods, order, cartEntity.getCount(), originPrice);
                        ...
                        /*当所有商品 设置的除成本价以外的价格都是0*/
                        if (count>=shoppingCartEntities.size()){
                            orderEntity.setPriceType("4");
                        }else {
                            orderEntity.setPriceType(oriPriceType);
                        }
...
//            zxGood = goodsService.findById(goodsId);
    ...
    public OrderEntity addOrderEntity(OrderEntity orderEntity, Long shopId, String shopName, MemberEntity memberEntity,
                                      MemberAddressEntity memberAddressEntity, BigDecimal goodsAmount, Long payPoint, AgentFac agentFac) {
        BigDecimal shippingCost = new BigDecimal(0);
        ...
        //生成订单
        orderEntity.setUserId(agentFac.getUserId());//业务员编号
        ...
        try {
            ...
            //在订单生成的情况下，将此消息存入数据库中，用于工厂端提示
            SysMessage sysMessage = new SysMessage();
            long l = System.currentTimeMillis();
            Date date = new Date(l);
            SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            sysMessage.setContent("【直链网】代理人"+agentFac.getAgentName()+"，您于"+ simpleDateFormat.format(date) +"时，创建订单。");
            sysMessage.setTitle("创建订单");
            sysMessage.setAgentId(agentFac.getAgentId());
            //0代表工厂端，1代表商城端
            sysMessage.setMsgtype(1);
            sysMessage.setStatus(0);
            sysMessage.setSysId(shopId.intValue());
            sysMessage.setMemberId(memberEntity.getId().intValue());
            sysMessage.setFactId(agentFac.getFactoryId());
            sysMessage.setCreateTime(DateTool.parseDateTime(simpleDateFormat.format(date)));
            sysMessage.setSnType(1);
            sysMessage.setSn(orderNumber);
            //将申请代理商的消息同步到消息表中，为app端提供支持
     ...
     * 添加订单商品信息
     */
    public void addOrderGoods(GoodsEntity goods, OrderEntity order, int buyNum, BigDecimal originPrice) {
        if (order != null) {
            BigDecimal bigBuyNum = new BigDecimal(buyNum);
            //订单商品的信息
            OrderGoodsEntity orderGoods = new OrderGoodsEntity();
            ...
            orderGoods.setCommentStatus(0);
            orderGoods.setStatus(0);
            orderGoods.setOrigin_price(originPrice);
            orderGoods.setCreateDate(new Date());
...
//                }
                //插入系统消息
                sysMessageService.insertOrderSysMsg(order, memberEntity, 1,order.getShopId().intValue());
                r = true;
            }
...
//                }
                //插入系统消息
                sysMessageService.insertOrderSysMsg(order, memberEntity, 2,order.getShopId().intValue());
            }
