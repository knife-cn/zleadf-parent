		...
		if(evt.data.length == 0) {
	  	...
	  	//购物车不为空
	  		...
	  		for(var i = 0; i < evt.data.length; i++) {
	  			...
	  			htmlStr+="<ul>";
	  				for(var j = 0; j < evt.data[i].goodsList.length; j++){
	  					...
	  					htmlStr+="<img src='http://116.62.124.171/group1/"+evt.data[i].goodsList[j].goodsImg+"' alt=''>";
	  					htmlStr+="</div>";
	  					...
	  					htmlStr+="<p class='carList_price_1'>"+parseFloat((evt.data[i].goodsList[j].goodsPrice)*(evt.data[i].goodsList[j].count)).toFixed(2)+"</p>";
	  		...
	  		$(".min").click(function() {
                ...
                var h = $(this).parent().find('.shoppingCartId');
                ...
                var hvalue=Number(h.val());
	  			var evalue=Number(e.text());
	  			var fvalue=Number(f.text());
				weburl=baseUrl+"/zlead/tshoppingcart/updateCount";
                $.ajax({
                    url:weburl,
                    type: "post",
                    data: {
                    ...
                    success: function(res) {
						if(res.code==1){
                            if (tvalue > 1) {
                                tvalue -= 1;
                                t.val(tvalue);
                                f.text(parseFloat(tvalue * ((evalue) * 100) / 100).toFixed(2));
                            }else if(tvalue == 1){
                                tvalue = 1;
                                t.val(tvalue);
                                f.text(parseFloat(tvalue * ((evalue) * 100) / 100).toFixed(2));
            ...
            $(".amount").on('focusout', function() {
			  ...
			  $(".detCar").click(function() {
				...
				if(shoppingCartIdStr != "") {
					weburl=baseUrl+"/zlead/tshoppingcart/deleteShoppingCart";
					...
					appSupport.cm.postAjaxFunction(weburl, rigon, showModify);
			  ...
			  $(".buy").click(function(){
				 ...
				 if(shoppingCartIdStr != "" || goodsId !="" || num !="") {
					 /*var orderInfo = [{
						 goodsId: goodsId,
						 num:num
					 }];*/
					...
					/* goodsId = JSON.stringify(goodsId); 
					 ...
					 weburl=baseUrl+"/zlead/torder/cartComfirmOrder";
					$.ajax({
						url: weburl,
				        type: 'post',
				        data: {
				        	"id":shoppingCartIdStr
				        	/*"buyNum":num,*/
				        	/*"prodType":prodType*/
				        },
				        success: function(res) {
				        	if(res.code==1){
				        		window.location.href = "agentAffirmOrder.jsp?id="+shoppingCartIdStr+"&goodsId="+goodsId+"&channel="+5+"&buyNum="+num+"&status="+2;
				        	}
				        }
					})
				 } else {
...
function changeTab(obj, index) {
    ...
    $(".pointType").val(prodType);
    var shopId=appSupport.cm.queryString("shopId");
    if (!shopId){
    	shopId=localStorage.getItem("shopId");
	}
	var baseUrl=localStorage.getItem("weburl");
	var weburl=baseUrl+"/zlead/tshoppingcart/shoppingCartGoods";
    //var url = "../../zlead/tshoppingcart/shoppingCartGoods";
	var data = {
			"type":0,
			"prodType":prodType,
			"channelType":2,
	...
	appSupport.cm.postAjaxFunction(weburl, data, showShopCar);
 }
