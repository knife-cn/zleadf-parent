...
function orderList() {
	...
	var weburl=baseUrl+"/zlead/torder/cartComfirmOrder";
    if(status==2){
        ...
        var html='';
		weburl=baseUrl+"/zlead/torder/newConfirmOrder";
        $.ajax({
            url: weburl,
            type: 'post',
            data: {
...
function goPay(){
	...
	var baseUrl=localStorage.getItem("weburl");
	var weburl=baseUrl+"/zlead/torder/addOrder";
    ...
    if(status==1){
            ...
            success: function(res) {
                if(res.code==1){
                    ...
                    setTimeout(function() {
                        ...
                        window.location.href = baseUrl+"/member/agentOrderPay?orderNo="+orderNo+"&status="+1;
                    }, 1000);
                }else{
		...
		weburl=baseUrl+"/zlead/torder/cartAddOrder";
        $.ajax({
            url: weburl,
            type: 'post',
            data: {
            ...
            success: function(res) {
                if(res.code==1){
                    ...
                    setTimeout(function() {
                        appSupport.cm.errorMessageHide(errorMsg);
                        var orderNo=res.data.orderSn;
                        window.location.href = baseUrl+"/member/agentOrderPay?orderNo="+orderNo+"&status="+2;
                    }, 1000);
                }else{
                    $(".buy img").attr("onclick","goPay()");
                }
