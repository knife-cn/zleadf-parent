        ...
        success: function(res) {
            if(res.code=1){
                ...
                html+='<div class="total">'
                    ...
                    +'<span class="totalSpan">RMB<label id="totalPrice">'+res.data.payableAmount+'</label></span>'
                    +'</div>'
                    +'<div class="buy">'
        ...
        if (totalAmount!=0){
            if($('input[name="zf"]').is(':checked')==true){
                if(type==1){
                    ...
                    window.location.href = "../zlead/pay/aliPagePay?orderNo="+orderNo+"&totalAmount="+totalAmount;
                }else if(type==2){
                    var msg = "微信支付尚未开通！";
                    ...
                    $.ajax({
                        url: '../ajeasy/allianceBusiness/voucherPay',
                        type: 'get',
                        data: {
                            "orderNo":orderNo
                        },
                        success: function(res) {
                            if(res.code==1){
                                ...
                                setTimeout(function() {
                                    appSupport.cm.errorMessageHide(errorMsg);
                                    window.location.href = "alliance.jsp";
                                }, 1000);
                            }else{
                                var msg = res.message;
                                appSupport.cm.errorMessageShow(errorMsg, msg);
        ...
        if (totalAmount!=0){
            if($('input[name="zf"]').is(':checked')==true){
                if(type==1){
                    ...
                    window.location.href = "../zlead/pay/aliPagePay?paySn="+paySn+"&totalAmount="+totalAmount;
                }else if(type==2){
                    var msg = "微信支付尚未开通！";
                    appSupport.cm.errorMessageShow(errorMsg, msg);
                    setTimeout(function() {
