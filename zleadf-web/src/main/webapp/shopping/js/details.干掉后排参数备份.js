$(function() {
        ...
        if (mystore && mystore.length > 0) {
            for (var i = 0; i < mystore.length; i++) {
                ...
                for (var j = 0; j < $(".avtie").length; j++) {
                    ...
                    if (mystore[i].attr_val.indexOf(str) == -1) {
                        flag = false;
                    }
                }
                if (flag) {
    ...
    $('#quantity').keyup(function() {
        if (parseInt($(this).val()) > parseInt($(this).parents('.dt_shu').find('#dt_glespas').attr('goodsnum'))) {
            $(this).val(parseInt($(this).parents('.dt_shu').find('#dt_glespas').attr('goodsnum')))
        }
                ...
                if (res.data != null) {
                    ...
                    $("#star").click(function() {
                        ...
                        $.ajax({
                            type: "post", //提交请求的方式
                            cache: true, //是否有缓存
                            url: "/zlead/fplat/saveCollect/" + shoppingid + "", //访问servlet的路径
                            dataType: "json", //没有这个，将把后台放会的json解析成字符串
                            data: {
                    ...
                    $("#scx").click(function() {
                        $.ajax({
                            type: "post", //提交请求的方式
                            cache: true, //是否有缓存
                            url: "/zlead/fplat/deleteCollect/" + shoppingid + "", //访问servlet的路径
                            dataType: "json", //没有这个，将把后台放会的json解析成字符串
                            data: {
                                ...
                                success: function(res) {
                                    ...
                                    if (newArr.length > 0) {
                                        ...
                                        for (var i = 0; i < newArr.length; i++) {
                                            var flag2 = true;
                                            var arr = newArr[i].attr_val.split('_');
                                            // 当前选中之后的参数
                                            for (var j = index + 1; j < leng; j++) {
                                                var myflag3 = true;
                                                if ($('.dt_yul').eq(j).find('.avtie')) {
                                                    var str2 = $('.dt_yul').eq(j).find('.avtie').find('p').html();
                                                    for (var k = 0; k < arr.length; k++) {
                                                        if (str2 == arr[k]) {
                                                            myflag3 = false;
                                                        }
                                        ...
                                        if (newArr2.length == 1) {
                                            ...
                                            // if ($(".dt_yul").length == $(".avtie").length) { // 是否全部参数选中状态
                                            if (res.data.goodsDetailDto.prodType == 2) { // 走活动详情
                                                // 3.25修改：后端不通过actId判断是否为活动商品了，而是根据res.data.goodsDetailDto.prodType
                                                // 活动价格
                                                if (newArr2[0].if_show_price == 1) {
                                                    $('.dt_pt').find('span').eq(0).html('¥' + newArr2[0].show_price.toFixed(2)); // 当前参数价格-活动商品详情页
                                                    $('.dt_pt').find('span').eq(1).html('¥' + newArr2[0].resultPrice.toFixed(2)); // 当前参数价格-活动商品详情页

                                                }

                                                // 活动库存
                                                if (newArr2[0].if_show_stock == 1) {
                                                    $('#dt_glespas').html(newArr2[0].stock).attr('goodsNum', newArr2[0].stock) //库存数量
                                                } else {
                                                    $('#dt_glespas').html("--").attr('goodsNum', newArr2[0].stock) //库存数量
                                                }

                                            } else { // 普通商品
                                                // 普通价格
                                                if (newArr2[0].if_show_price == 1) {
                                                    $('.dt_pt').find('span').eq(0).html('¥' + newArr2[0].show_price.toFixed(2)); // 当前参数价格-普通商品详情页
                                                } else {
                                            ...
                                            for (var j = leng - 1; j > index; j--) {
                                                var hello = [];
                                                $('.avtie').eq(j).removeClass("avtie").addClass("a").find(".gou").hide();
                                                // 判断取消最后一个选中状态后，当前参数的型号是否有
                                                for (var i = 0; i < myarr.length; i++) {
                                                    var flag = true;
                                                    var arr = myarr[i].attr_val.split('_');
                                                    // 当前之前的参数
                                            ...
                                            for (var i = 0; i < newArr2.length; i++) {
                                                var mynarr = newArr2[i].attr_val.split('_');
                                                if (mynarr.length == $(".avtie").length) { // 过滤：过滤参数数量不相等
                                                    haha.push(newArr2[i]);
                                            ...
                                            if (haha.length == 1) {
                                                if (res.data.goodsDetailDto.prodType == 2) { // 走活动详情
                                                    // 3.25修改：后端不通过actId判断是否为活动商品了，而是根据res.data.goodsDetailDto.prodType
                                                    // 活动价格
                                                    if (haha[0].if_show_price == 1) {
                                                        $('.dt_pt').find('span').eq(0).html('¥' + haha[0].show_price.toFixed(2)); // 当前参数价格-活动商品详情页
                                                        $('.dt_pt').find('span').eq(1).html('¥' + haha[0].resultPrice.toFixed(2)); // 当前参数价格-活动商品详情页

                                                    }

                                                    // 活动库存
                                                    if (haha[0].if_show_stock == 1) {
                                                        $('#dt_glespas').html(haha[0].stock).attr('goodsNum', haha[0].stock) //库存数量
                                                    } else {
                                                        $('#dt_glespas').html("--").attr('goodsNum', haha[0].stock) //库存数量
                                                    }

                                                } else { // 普通商品
                                                    // 普通价格
                                                    if (haha[0].if_show_price == 1) {
                                                        $('.dt_pt').find('span').eq(0).html('¥' + haha[0].show_price.toFixed(2)); // 当前参数价格-普通商品详情页
                                                    } else {
                                                        $('.dt_pt').find('span').eq(0).html('¥--'); // 当前参数价格
                                                    }
                                        ...
                                        // $('#dt_glespas').html('').attr('goodsNum', 0).hide()
                                        // changeColor()
                                    }
                                }
                            })
                    ...
                    function changeColor() {
                        ...
                        if (newArr.length == 0) {
                            ...
                            $('.dt_pt').find('span').html('');
                            $('#dt_glespas').html('').attr('goodsNum', 0);
                        } else if (parseInt($('#quantity').val()) > newArr[0].stock && parseInt($('#quantity').val()) > 0) { //库存不足
                            $("#dtgocart").css({
                                background: 'rgb(204, 204, 204)'
                            });
                            $('#payfor2').css({
                        ...
                        // if ($(".dt_yul").length != $(".avtie").length) { // 判断当前是否所有参数都选中
                        //     $("#dtgocart").css({
                        //         background: 'rgb(204, 204, 204)'
                        //     });
                        //     $('#payfor2').css({
                        //         background: 'rgb(204, 204, 204)'
                        //     });
                        // } else if (parseInt($('#quantity').val()) > parseInt($('#dt_glespas').attr('goodsNum')) && parseInt($('#quantity').val()) > 0) { //库存不足
                        //     $("#dtgocart").css({
                        //         background: 'rgb(204, 204, 204)'
                        //     });
                        //     $('#payfor2').css({
